    ...
    for img_name in file_list_img:
        ...
        while True:
            ...
            def Map(k):
                ...
                img = Image.open('/home/gangmin/experiment/data/grid_images_v2/'+img_shape+"/"+str(OriginSegments)+'/'+str(overlap)+'/'+ k[0] +'.jpg')
            ...
            dir = '/home/gangmin/experiment/mrslic/exp5/'+img_shape+'/'+str(OriginSegments)+'/'+str(overlap)
            if not os.path.exists(dir):
                os.makedirs(dir)
            df = pd.DataFrame(labels[0])
            df.to_csv(dir+'/'+img_name[:-4]+".csv",header=None,index=None)
            overlap +=1
